...
module.exports = class ResourceD {
    ...
    static async updateById(id, updateObj){
        ...
        const currentValue = updateObj.currentValue ?? getByIdResponse.currentValue;
        
        const { rows } = await pool.query(
            'UPDATE resource_d SET name=$1, description=$2, current_value=$3', [name, description, currentValue]
        )
        return new ResourceD(rows[0]);
    }
}